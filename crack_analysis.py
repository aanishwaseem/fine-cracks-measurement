...
class CrackAnalyse(object):
    def __init__(self, predict_image_file=None, predict_image_array=None, scaling_factor_x=1.0, scaling_factor_y=1.0):
        ...
        if predict_image_array is not None:
            img = predict_image_array
        else:
            img = imageio.imread(predict_image_file)
        if img.ndim == 3:
            img = img[:, :, 0]
        self.sfx = scaling_factor_x
        self.sfy = scaling_factor_y
        ...
        for label in labels:
            ...
            coords = np.column_stack(np.nonzero(median_axis))
    ...
    def get_crack_max_width_mm(self):
        """Return max crack width in mm"""
        max_width_px = np.max(self.crack_max_wids)
        return max_width_px * (self.scaling_factor_x + self.scaling_factor_y) / 2

    def get_crack_mean_width_mm(self):
        """Return mean crack width in mm"""
        mean_width_px = np.sum(self.img_bnr) / np.sum(self.crack_lens)
        return mean_width_px * (self.scaling_factor_x + self.scaling_factor_y) / 2
    
    def get_crack_length_mm(self):
        # optionally use dx/dy to compute "real" length in mm
        lengths_mm = [np.sqrt((c['dx']*self.sfx)**2 + (c['dy']*self.sfy)**2) for c in self.cracks_info]
        return np.sum(lengths_mm)

    def get_crack_max_length_mm(self):
        lengths_mm = [np.sqrt((c['dx']*self.sfx)**2 + (c['dy']*self.sfy)**2) for c in self.cracks_info]
        return max(lengths_mm)
